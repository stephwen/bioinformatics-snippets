...
my $annovarDir = "/home/volatile/swe/soft/annovar/";

my $prefix = dirname($bam)."/".basename($bam, ".bam")."_SNPCall";
...
push(@{$commands{2}{'cmd'}}, "java -Xmx4g -jar $GATKExe -R $ref -T UnifiedGenotyper -I $bam -glm BOTH --dbsnp $dbSNPVCF -o $rawVCF -dcov 200");
...
push(@{$commands{5}{'cmd'}}, "$annovarDir/convert2annovar.pl $recalFilteredVCF -format vcf4 > $avinputFile");
push(@{$commands{5}{'cmd'}}, "$annovarDir/summarize_annovar.pl --buildver hg19 --verdbsnp 135 --ver1000g 1000g2012apr --veresp 6500 --outfile $annovarPrefix $avinputFile $annovarDb");
